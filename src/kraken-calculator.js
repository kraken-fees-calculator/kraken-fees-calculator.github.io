...
module.exports = {
  ...
  getMarginFees: function(options) {
    if (options.trade.leverage == 1) {
      ...
      return {
        opening: (real_leverage * options.trade.amount * options.trade.price_in * options.fees.opening / 100),
        rollover: (real_leverage * options.trade.amount * options.trade.price_in * options.fees.rollover / 100 * Math.floor(duration/4))
      }
    };
  },
